...
static bool skin_is_goku = false; /* true if current skin is goku/very-fast-type */
...
void ChangePlayerSkin(const char* idle_path, const char* walk_path, const char* jump_path)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if(strstr(p, "GOKU")){
            suggested_speed = default_base_player_speed * 2.5f; /* extremely fast */
            found_goku = true;
            break;
        }
        if (strstr(p, "SONIC")) {
    ...
    if (jump_player.tex_jumpplayer) {
        ...
        if (frame_w <= 0) frame_w = w;
        detected_frames = (frame_w > 0) ? (w / frame_w) : 1;
        if (detected_frames <= 0) detected_frames = 1;
        frame_h = idle_frame_h ? idle_frame_h : h;
        Animation_Init(&jump_player.jump_anim, frame_w, frame_h, detected_frames, FRAME_DURATION_PLAYER);
        SDL_Log("ChangePlayerSkin: jump texture %dx%d -> frames=%d frame_size=%dx%d\n", w, h, detected_frames, frame_w, frame_h);
        if (frame_w != PLAYER_WIDTH || frame_h != PLAYER_HEIGHT) {
            SDL_Log("Note: jump frame size differs from PLAYER_WIDTH/HEIGHT (%d x %d)\n", PLAYER_WIDTH, PLAYER_HEIGHT);
    ...
    */
    ...
    if (found_metal) {
        ...
        ChangePlayerShootSkin("sprites/SHOOTGOKU.png");
        ChangePlayerShootWalkSkin("sprites/SHOOTGOKU.png");
        SDL_Log("ChangePlayerSkin: auto-loaded shoot spritesheets for GOKU skin\n");
    }else{
...
float GetShiftMultiplierForSkin(void)
{
    ...
    if (skin_is_metal) return 2.0f;
    if (skin_is_goku) return 2.5f; /* extremely fast */
    return 1.8f; /* reduced sprint multiplier for base/non-metal skins */
}
...
float GetProjectileDamageMultiplier(void)
{
    ...
    if (skin_is_goku) return 2.0f; /* double damage */
    return 1.0f;
}
...
void PlayerWalkAnim(SDL_RendererFlip flip_type)
{
    ...
    if (skin_is_goku) {
        ...
        walk_player.dest_walkplayer.y = position_y + (PLAYER_HEIGHT - src_mod.h);
...
void PlayerJumpAnim(SDL_RendererFlip flip_type)
{
    ...
    if (skin_is_goku) {
        ...
        const int left_extra = -5;  /* pixels to include to the left of frame */
        const int right_crop = 0;  /* pixels to remove from right of frame */
